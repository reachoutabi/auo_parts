...
def get_mobile_accessible_header(active_file):
    ...
    home_btn_cls = "flex items-center gap-1 font-bold text-red-500 border-b-2 border-red-500 pb-0.5 px-1 xl:px-1.5 focus:outline-none whitespace-nowrap" if is_home else "flex items-center gap-1 text-slate-200 hover:text-red-400 px-1 xl:px-1.5 py-0.5 rounded transition-colors whitespace-nowrap focus:outline-none"
    products_cls = "text-red-500 font-bold border-b-2 border-red-500 pb-0.5 px-1 xl:px-1.5 rounded transition-colors whitespace-nowrap" if is_products else "text-slate-200 hover:text-red-400 px-1 xl:px-1.5 py-0.5 rounded transition-colors whitespace-nowrap"
    finder_cls = "text-red-500 font-bold border-b-2 border-red-500 pb-0.5 px-1 xl:px-1.5 rounded transition-colors whitespace-nowrap" if is_finder else "text-slate-200 hover:text-red-400 px-1 xl:px-1.5 py-0.5 rounded transition-colors whitespace-nowrap"
    brands_cls = "text-red-500 font-bold border-b-2 border-red-500 pb-0.5 px-1 xl:px-1.5 rounded transition-colors whitespace-nowrap" if is_brands else "text-slate-200 hover:text-red-400 px-1 xl:px-1.5 py-0.5 rounded transition-colors whitespace-nowrap"
    bulk_cls = "text-red-500 font-bold border-b-2 border-red-500 pb-0.5 px-1 xl:px-1.5 rounded transition-colors whitespace-nowrap" if is_bulk else "text-slate-200 hover:text-red-400 px-1 xl:px-1.5 py-0.5 rounded transition-colors whitespace-nowrap"
    about_cls = "text-red-500 font-bold border-b-2 border-red-500 pb-0.5 px-1 xl:px-1.5 rounded transition-colors whitespace-nowrap" if is_about else "text-slate-200 hover:text-red-400 px-1 xl:px-1.5 py-0.5 rounded transition-colors whitespace-nowrap"
    blog_cls = "text-red-500 font-bold border-b-2 border-red-500 pb-0.5 px-1 xl:px-1.5 rounded transition-colors whitespace-nowrap" if is_blog else "text-slate-200 hover:text-red-400 px-1 xl:px-1.5 py-0.5 rounded transition-colors whitespace-nowrap"
    contact_cls = "text-red-500 font-bold border-b-2 border-red-500 pb-0.5 px-1 xl:px-1.5 rounded transition-colors whitespace-nowrap" if is_contact else "text-slate-200 hover:text-red-400 px-1 xl:px-1.5 py-0.5 rounded transition-colors whitespace-nowrap"
        ...
        <div class="max-w-[1440px] mx-auto px-2 sm:px-4 py-2">
            <div class="flex items-center justify-between gap-2 sm:gap-4">
                ...
                <a href="index.html" class="flex items-center gap-1.5 sm:gap-2 group flex-shrink-0">
                    ...
                    <div>
                        <div class="text-base sm:text-xl font-black tracking-tight text-white flex items-center gap-1 leading-tight">
                            AUTO<span class="text-red-600">PARTS</span>
                        </div>
                        <div class="text-[8px] sm:text-[10px] uppercase font-bold text-slate-400 tracking-widest leading-none">PRO STOREFRONT</div>
                    </div>
                </a>

                <!-- Desktop Nav Links (Visible on lg and up) -->
                <nav class="hidden lg:flex items-center gap-1 xl:gap-3 text-[11px] xl:text-xs font-semibold whitespace-nowrap flex-shrink-0">
                    <!-- Home Dropdown Menu -->
                ...
                <!-- Header Actions (Compare, Wishlist, Theme, RTL, Auth, Cart, Mobile Menu) -->
                <div class="flex items-center gap-1 sm:gap-1.5 xl:gap-2 flex-shrink-0">
                    <!-- Compare Button (Visible sm and up) -->
                    <button type="button" onclick="AutoPartsUI.openCompareModal()" class="hidden sm:inline-flex relative p-1.5 sm:p-2 text-slate-300 hover:text-red-400 transition-colors flex-shrink-0" title="Compare Products">
                        <i class="fa-solid fa-code-compare text-sm sm:text-lg"></i>
                    ...
                    <!-- Wishlist Button -->
                    <a href="account.html#wishlist" class="relative p-1.5 sm:p-2 text-slate-300 hover:text-red-400 transition-colors flex-shrink-0" title="Wishlist">
                        <i class="fa-solid fa-heart text-sm sm:text-lg"></i>
                    ...
                    <!-- Theme Switcher Button (Visible sm and up) -->
                    <button type="button" onclick="AutoPartsUI.toggleTheme()" class="hidden sm:inline-flex p-1.5 sm:p-2 text-slate-300 hover:text-amber-400 transition-colors flex-shrink-0" title="Toggle Dark/Light Theme" aria-label="Toggle Dark/Light Theme">
                        <i class="fa-solid fa-moon dark:hidden text-sm sm:text-lg text-amber-400"></i>
                        <i class="fa-solid fa-sun hidden dark:inline text-sm sm:text-lg text-amber-400"></i>
                    </button>

                    <!-- RTL Switcher Button (Visible sm and up) -->
                    <button type="button" onclick="AutoPartsUI.toggleRTL()" class="hidden sm:inline-flex p-1.5 sm:p-2 text-slate-300 hover:text-blue-400 transition-colors flex-shrink-0" title="Toggle LTR/RTL Layout" aria-label="Toggle LTR/RTL Layout">
                        <i class="fa-solid fa-right-left text-sm sm:text-lg text-blue-400"></i>
                    </button>

                    <!-- Login / Account Link -->
                    <a href="login.html" class="js-auth-top-link p-1.5 sm:p-2 text-slate-300 hover:text-red-400 transition-colors flex-shrink-0" title="Log In to your Account">
                        <i class="fa-solid fa-right-to-bracket text-sm sm:text-lg"></i>
                    </a>

                    <!-- Cart Drawer Trigger Button (My Cart) -->
                    <button type="button" onclick="AutoPartsUI.toggleCartDrawer(true)" class="flex items-center gap-1.5 sm:gap-2 bg-slate-800 hover:bg-red-600 text-white px-2 sm:px-3 py-1.5 rounded-xl transition-all font-semibold text-xs border border-slate-700 flex-shrink-0">
                        <div class="relative">
                    ...
                    <!-- Prominent Mobile Menu Hamburger Button (Always visible & accessible below lg) -->
...
for filepath in html_files:
    ...
    if header_regex.search(content):
        header_html = get_mobile_accessible_header(filename)
        new_content = header_regex.sub(header_html, content)
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(new_content)
        print(f"Updated mobile accessible header in {filename}")
